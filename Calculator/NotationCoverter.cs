...
{
    public static class NotationCoverter
    {
		/// <summary>
		...
		private static readonly Dictionary<char, int> Operators = new Dictionary<char, int>
	{
		...
		/// <exception cref="Exception">Thrown if the infix expression is invalid.</exception>
		/// <exception cref="DivideByZeroException">Thrown if the infix expression attempts to divide by 0.</exception>
		...
		/// <exception cref="Exception">Thrown if the infix expression is invalid.</exception>
		/// <exception cref="ArgumentException">Thrown if the infix expression is null, empty, or consists only of whitespace characters.</exception>
		...
		public static string InfixToPrefix(string infix)
		{
			...
			var output = new StringBuilder();
			var tempOperand = new StringBuilder();
			foreach (char token in Regex.Replace(infix, @"\s+", ""))
			{
				if (Operators.ContainsKey(token))
				{
					HandlePrefixOperatorCase(token, operatorStack, operands, tempOperand);
				}
				else
				{
					switch (token)
		...
		/// <exception cref="Exception">Thrown if two operators are adjacent.</exception>
		private static void HandleOperatorCase(char operatorToken, Stack<char> operatorStack, StringBuilder output)
		{
			...
			{
				throw new Exception($"Operators {operatorStack.Peek()} and {operatorToken} are adjacent.");
			}
		...
		private static void HandlePrefixOperatorCase(char operatorToken, Stack<char> operatorStack, Stack<String> operands, StringBuilder tempOperand)
		{
			// The operator case is the only case where a space is appended to the output string ahead of
			// the next iteration. Thus, if the last character in the output string is a space, then two
			// operators must be adjacent to one another.
			//if (tempOperand.Length > 0 && tempOperand[tempOperand.Length - 1] == ' ')
			//{
			//	throw new Exception($"Operators {operatorStack.Peek()} and {operatorToken} are adjacent.");
			//}
			tempOperand.Append(" ");
		...
		/// <param name="operatorStack">The shunting-yard algorithm's operator stack.</param>
		/// <param name="output">The shunting-yard algorithm's current output string.</param>
		/// <exception cref="Exception">Thrown if a matching left parenthesis is not found.</exception>
		...
		/// <param name="operatorStack">The shunting-yard algorithm's operator stack.</param>
		/// <param name="output">The shunting-yard algorithm's current output string.</param>
		/// <exception cref="Exception">Thrown if a left parenthesis is found on the stack.</exception>
		...
		/// <exception cref="Exception">Thrown if the postfix expression is invalid.</exception>
		/// <exception cref="ArgumentException">Thrown if the postfix expression is null, empty, or consists only of whitespace characters.</exception>
		/// <exception cref="DivideByZeroException">Thrown if the postfix expression attempts to divide by 0.</exception>
		...
		/// <exception cref="Exception">Thrown if operatorToken is unsupported.</exception>
		/// <exception cref="DivideByZeroException">Thrown if the expression attempts to divide by 0.</exception>
		private static void ApplyOperatorToOperands(char operatorToken, decimal operand1, decimal operand2, Stack<decimal> stack)
		{
			switch (operatorToken)
			{
				...
				case '*':
					...
					break;
				default:
					throw new Exception($"{operatorToken} is an unsupported operator.");
			}
		}
